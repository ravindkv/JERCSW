...
RunL2ResidualZmmJet::RunL2ResidualZmmJet(const GlobalFlag& globalFlags)
  : globalFlags_(globalFlags)
{
    loadConfig("config/RunL2ResidualZmmJet.json");
}

void RunL2ResidualZmmJet::loadConfig(const std::string& filename) {
...
auto RunL2ResidualZmmJet::Run(std::shared_ptr<SkimTree>& skimT,
                      ...
                      TFile*               fout) -> int
{
    ...
    for (Long64_t jentry = 0; jentry < nentries; ++jentry) {
        ...
        if (globalFlags_.isMC()){
            auto sfs =  scaleMuon->getMuonSfs(*skimT, pickedMuons[0], 
